...
from wildspeech.diarization.uisrnn import loss_func, utils
...
class UISRNN:
    ...
    def fit_concatenated(self, train_sequence, train_cluster_id, args):
        ...
        if (not isinstance(train_sequence, np.ndarray) or
            ...
            print('train_sequence type: {}'.format(type(train_sequence)))
            print('train_sequence dtype: {}'.format(train_sequence.dtype))
    ...
    def predict_single(self, test_sequence, args):
        ...
        beam_set = [BeamState()]
        iter_range = np.arange(0, args.test_iteration * test_sequence_length, args.look_ahead)
        for num_iter in tqdm(iter_range):
            with torch.no_grad():
    ...
    def predict(self, test_sequences, args, debug=False):
        ...
        if isinstance(test_sequences, list):
            if debug:
                return [self.predict_single(test_sequence, args)
                        for test_sequence in tqdm(test_sequences)]
            else:
                return [self.predict_single(test_sequence, args)
