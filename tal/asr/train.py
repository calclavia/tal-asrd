...
if __name__ == '__main__':
    ...
    if args.load_decoder:
        print('Loading decoder weights...', args.load_decoder)
        checkpoint = torch.load(
            args.load_decoder, map_location=lambda storage, loc: storage)
        state_dict = checkpoint['state_dict']

        # Filter out only decoder parts
        print('!!! WARNING: Only loading decoder parts of the model!')
        # state_dict = {k:v for k, v in state_dict.items() if '.decoder' in k}
        # Expand embedding by number of speares
        embedding = state_dict['model.embedding.weight']
        state_dict['model.embedding.weight'] = torch.cat((
            embedding,
            torch.empty(args.num_speakers, embedding.size(
                1)).uniform_(- 1 / embedding.size(1), 1 / embedding.size(1))
        ))
        print('Expanded embedding to:',
              state_dict['model.embedding.weight'].size())
        del state_dict['model.lm_head.weight']

        print('Loading decoder parameters from', list(state_dict.keys()))
        model.load_state_dict(state_dict, strict=not args.no_strict)
        # give model a chance to load something
        model.on_load_checkpoint(checkpoint)
