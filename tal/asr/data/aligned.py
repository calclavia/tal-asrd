...
def build_index(data_dir: str, file_stub: str, utterances: list,
    ...
    if num_utterances is None:
        ...
        index.append((file_stub, utterances, f_duration))
    else:
        valid_utts = [
            u for u in utterances if is_valid_utterance(u, f_duration)]
...
class ASRAlignedDataset(Dataset):
    """
    Audio -> Text dataset.
    ...
    def __init__(self,
        ...
        Args:
            ...
            tokenizer_speakers: Should we turn speakers into tokens?
            return_spk_ids: Should the dataset return speaker IDs?
        """
        super().__init__()
    ...
    def __getitem__(self, i: int):
        ...
        # Load audio
        start_time = utterances[0]['utterance_start']
        end_time = utterances[-1]['utterance_end']
        ...
        assert len(utterance_tokens) > 1
        text = torch.LongTensor(utterance_tokens)

        if spk_ids is None:
            spk_ids = torch.LongTensor(len(text) * [0])
        else:
...
class ASRAlignedCollater:
    ...
    def __call__(self, samples):
        ...
        raw_audio, text, spk_ids, idx = zip(*samples)
        ...
        spk_ids = torch.stack([torch.nn.functional.pad(
            x, (0, max_text_len - len(x)), value=self.padding_idx) for x in spk_ids])

        return audio, audio_lens, text, text_mask, spk_ids, idx
